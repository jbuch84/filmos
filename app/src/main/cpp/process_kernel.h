    ...
    int16_t* chroma_work)
{
    ...
    if (is_yuv) {
        ...
        for (int x = 0; x < width; x++) {
            ...
            int cb_orig = (-43 * r - 85 * g + 128 * b) >> 8;
            int cr_orig = (128 * r - 107 * g -  21 * b) >> 8;
            ...
            row[x * 3]     = (uint8_t)CLAMP(r + ((359 * dcr) >> 8));
            row[x * 3 + 1] = (uint8_t)CLAMP(g - ((88  * dcb) >> 8) - ((183 * dcr) >> 8));
            row[x * 3 + 2] = (uint8_t)CLAMP(b + ((453 * dcb) >> 8));
        }
